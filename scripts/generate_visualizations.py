...
def create_steampunk_dashboard(stats):
    ...
    try:
        ...
        ax_bg.imshow(
            gradient, extent=extent, aspect="auto", cmap="Blues_r", alpha=0.4, zorder=0
        )
        ax_bg.set_xlim(0, 1)
        ax_bg.set_ylim(0, 1)
        ax_bg.axis("off")
        ...
        if country_counts:
            ...
            for b in bars:
                ...
                tt.set_path_effects(
                    [pe.withStroke(linewidth=1.5, foreground="#0a0a2e")]
                )
            ax2.set_xticks(x)
        ...
        if city_rows:
            ...
            ax4.set_title(
                "Top Cities (bubble size = IP count)",
                color="#ff1493",
                fontweight="bold",
            )
...
def create_country_chart(stats):
    ...
    try:
        ...
        bars = ax.bar(
            x, counts, color=palette_colors, edgecolor="#00ffff", linewidth=1.5
        )
        ...
        ax.set_title(
            "Top 15 Countries with Malicious IPs",
            pad=16,
            color="#ffff00",
            fontweight="bold",
        )
...
def create_severity_chart(stats):
    ...
    try:
        ...
        ax.set_title(
            "Malicious IP Threat Severity Distribution",
            pad=16,
            color="#ff1493",
            fontweight="bold",
        )
...
def create_geo_map(stats):
    ...
    try:
        ...
        bars = ax.barh(
            y, counts, color=palette_colors, edgecolor="#00ffff", linewidth=1.5
        )
        ...
        ax.set_title(
            "Global Distribution of Malicious IPs (Top 20 Countries)",
            pad=16,
            color="#ffff00",
            fontweight="bold",
        )
...
def create_world_pins_map(stats):
    ...
    try:
        ...
        fig.update_layout(
            ...
            title=dict(
                text="Global Malicious IPs — Country Pins",
                x=0.5,
                font=dict(color="#ffaa00", size=18),
            ),
        )
